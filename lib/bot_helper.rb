...
module DiscordDatastore::BotHelper
  ...
  def get_channels
    server = get_server
    return server.channels if server
    []
  end

  def get_users
    server = get_server
    return server.members if server
    []
  end

  def get_ranks
    server = get_server
    return server.roles if server
    []
  end
  ...
  def upsert_user(user)
    ...
    u = DiscordDatastore::DiscordUser.find_by(id: user.id)
  ...
  def get_oldest_message_id(channel)
    ...
    return recent[0].id if recent.length > 0
  ...
  def parse_discord_messages(messages)
    messages.map do |message|
      attachments = []
      message.attachments.each { |attachment| attachments.push(attachment.url) }
  ...
  def browse_history
    ...
    get_channels.each do |channel|
      ...
      if channel.id == SiteSetting.discord_bot_channel_id.to_i &&
           SiteSetting.discord_ignore_bot_channel
      ...
      temp = 0
      loop do
        messages = channel.history(HISTORY_CHUNK_LIMIT, last_id)
      ...
      loop do
        messages = channel.history(HISTORY_CHUNK_LIMIT, nil, last_id)
        break if messages.length == 0
  ...
  def update_ranks
    ...
    rank_names.each do |rank_name|
      all_ranks.each { |rank| ranks.push(rank) if rank_name == rank.name }
    end
